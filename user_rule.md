...
### 2.3 数据库规范
- **ORM**：必须使用 ORM（Prisma、SQLAlchemy、TypeORM、GORM）管理数据，**严禁**拼接或直接执行原始 SQL 字符串，所有数据操作需通过 ORM 完成。
- **字符集规范（强制）**：数据库、数据表、ORM 连接层统一使用 `utf8mb4` 字符集与 `utf8mb4_unicode_ci` 排序规则，应用层与接口返回统一采用 UTF-8 编码，**必须彻底避免中文乱码问题**。
- **Seeding（数据填充）**：必须提供初始化脚本（Seed），项目启动或容器启动时自动填充演示数据，**拒绝“空库”交付**；初始化数据需包含中文内容，用于验证系统在真实场景下无中文乱码。QA 打开页面时应有内容可见。
